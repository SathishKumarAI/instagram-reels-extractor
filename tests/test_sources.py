...
from reels_scrap.sources import add_source, load_sources, poll_all
...
def _wire(monkeypatch, cfg, enumerate_ids, lands):
    ...
    def fake_pipeline(c, cfg_path, progress=None, refresh_index=True):
        urls = Path(c.source.urls_file).read_text(encoding="utf-8").split()
        for u in urls:
            rid = u.rstrip("/").rsplit("/", 1)[-1]
            if rid in lands:
                (c.data_dir / f"{rid}.json").write_text(json.dumps({"id": rid, "url": u}), encoding="utf-8")
        return [], None
...
def test_add_source_is_idempotent(tmp_path):
    ...
    url = "https://www.instagram.com/u/saved/topic-research/10000000000000001/"
    add_source(url, path=reg)
    add_source(url, path=reg)  # same url again
    rows = load_sources(reg)
    assert len(rows) == 1
    assert rows[0].name == "topic-research"  # slug derived from url
    assert rows[0].type == "collection"


def test_same_named_collections_disambiguated(tmp_path):
    reg = tmp_path / "sources.json"
    a = add_source("https://www.instagram.com/u/saved/ai/10000000000000003/", path=reg)
    b = add_source("https://www.instagram.com/u/saved/ai/10000000000000002/", path=reg)
    ...
    assert add_source("https://www.instagram.com/u/saved/ai/10000000000000003/", path=reg).name == "ai"
    assert len(load_sources(reg)) == 2


def test_dedup_and_dead_letter(cfg, tmp_path, monkeypatch):
    reg = tmp_path / "sources.json"
    add_source("https://www.instagram.com/u/saved/phd/10000000000000001/", path=reg)

    # AAA already downloaded; source currently has AAA, BBB, CCC
    (cfg.data_dir / "AAA.json").write_text(json.dumps({"id": "AAA"}), encoding="utf-8")
    ...
    state = json.loads((cfg.output_dir / "sources_state.json").read_text(encoding="utf-8"))
